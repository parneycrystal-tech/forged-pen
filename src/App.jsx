...
Main plot: ${project?.mainPlot||"none"}`;
    ...
    // A chapter told from a POV character other than the protagonist shouldn't have its content
    // measured against HER personal Goal/Fear/Wound/Backstory/Misbelief, that's not what the chapter
    // is testing. Shared world/tone/theme/plot fields still apply across every POV in the same story.
    const driftPOVScope=(project?.multiPOV&&knownPOVChar&&protagNameForCapture&&knownPOVChar.toLowerCase()!==protagNameForCapture.toLowerCase())
      ?` This chapter is written from ${knownPOVChar}'s POV, not ${protagNameForCapture}'s. Do NOT flag drift on the protagonist-personal fields (protagonist, protagonistGoal, protagonistDream, protagonistFear, protagonistWound, protagonistBackstory, protagonistMisbelief), since those describe ${protagNameForCapture} specifically, not ${knownPOVChar}. Only compare against the shared story-wide fields: supporting, antagonist, worldSetting, worldRules, worldMythology, worldBeliefs, worldDanger, worldTone, themes, mainPlot.`
      :"";
    const driftInstructions=skipDrift?"":`

Then, as Agnes, the meticulous record keeper, compare what this chapter reveals against the EXISTING STORY BIBLE above, field by field.${driftPOVScope} Only flag a field as drift if the existing entry is a real, substantial entry (not "none") AND this chapter appears to move in a meaningfully different direction from it, not just add consistent detail. Agnes's voice is direct, selective, specific, and slightly pointed. She never uses the words "inconsistency", "error", "problem", or "contradiction". She says "evolving", "different direction", "moving toward something new". She is not alarmed. She is precise.
...
Themes: ${project?.themes||"none"}
Main plot: ${project?.mainPlot||"none"}`;
    // Same POV-scoping as the main capture: a manually-scanned chapter told from a non-protagonist
    // POV shouldn't have its content measured against the protagonist's own personal fields.
    const manualScanChapter=(project?.chapters||[]).find(c=>c.num===chapterNum);
    const manualScanProtagName=getProtagName(project);
    const manualScanPOVChar=(manualScanChapter?.povCharacter||"").trim();
    const manualDriftPOVScope=(project?.multiPOV&&manualScanPOVChar&&manualScanProtagName&&manualScanPOVChar.toLowerCase()!==manualScanProtagName.toLowerCase())
      ?` This chapter is written from ${manualScanPOVChar}'s POV, not ${manualScanProtagName}'s. Do NOT flag drift on the protagonist-personal fields (protagonist, protagonistGoal, protagonistDream, protagonistFear, protagonistWound, protagonistBackstory, protagonistMisbelief), since those describe ${manualScanProtagName} specifically, not ${manualScanPOVChar}. Only compare against the shared story-wide fields: supporting, antagonist, worldSetting, worldRules, worldMythology, worldBeliefs, worldDanger, worldTone, themes, mainPlot.`
      :"";
    try{
      const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
        max_tokens:2000,
        system:`You are Agnes, the meticulous record keeper. Compare this chapter against the EXISTING STORY BIBLE, field by field.${manualDriftPOVScope} Only flag a field as drift if the existing entry is a real, substantial entry (not "none") AND this chapter appears to move in a meaningfully different direction from it, not just add consistent detail. Your voice is direct, selective, specific, and slightly pointed. Never use the words "inconsistency", "error", "problem", or "contradiction". Say "evolving", "different direction", "moving toward something new". Not alarmed. Precise. Never use em dashes. Respond ONLY with a JSON object.`,
  ...
  const runRetroactiveDriftScan=async()=>{
    ...
    for(const ch of chapters){
      ...
      if(chapterText.length>50){
        try{
          const existingBible=`Title: ${project?.title||"untitled"}\nGenre: ${project?.genre||""}\nSynopsis so far: ${project?.synopsis||"none"}\nProtagonist: ${project?.protagonist||"none"}\nGoal: ${project?.protagonistGoal||"none"}\nDream: ${project?.protagonistDream||"none"}\nFear: ${project?.protagonistFear||"none"}\nWound: ${project?.protagonistWound||"none"}\nBackstory: ${project?.protagonistBackstory||"none"}\nMisbelief: ${project?.protagonistMisbelief||"none"}\nSupporting characters: ${project?.supporting||"none"}\nAntagonist: ${project?.antagonist||"none"}\nSetting: ${project?.worldSetting||"none"}\nWorld rules: ${project?.worldRules||"none"}\nMythology & paranormal rules: ${project?.worldMythology||"none"}\nBeliefs vs reality: ${project?.worldBeliefs||"none"}\nWhat makes it dangerous: ${project?.worldDanger||"none"}\nTone: ${project?.worldTone||"none"}\nThemes: ${project?.themes||"none"}\nMain plot: ${project?.mainPlot||"none"}`;
          // Per-chapter POV scoping, same as live capture and the manual scan: don't measure a
          // non-protagonist POV chapter against the protagonist's own personal fields.
          const retroChapterObj=(project?.chapters||[]).find(c=>c.num===ch);
          const retroProtagName=getProtagName(project);
          const retroPOVChar=(retroChapterObj?.povCharacter||"").trim();
          const retroDriftPOVScope=(project?.multiPOV&&retroPOVChar&&retroProtagName&&retroPOVChar.toLowerCase()!==retroProtagName.toLowerCase())
            ?` This chapter is written from ${retroPOVChar}'s POV, not ${retroProtagName}'s. Do NOT flag drift on the protagonist-personal fields (protagonist, protagonistGoal, protagonistDream, protagonistFear, protagonistWound, protagonistBackstory, protagonistMisbelief), since those describe ${retroProtagName} specifically, not ${retroPOVChar}. Only compare against the shared story-wide fields: supporting, antagonist, worldSetting, worldRules, worldMythology, worldBeliefs, worldDanger, worldTone, themes, mainPlot.`
            :"";
          const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
            max_tokens:2000,
            system:`You are Agnes, the meticulous record keeper. Compare this chapter against the EXISTING STORY BIBLE, field by field.${retroDriftPOVScope} Only flag a field as drift if the existing entry is a real, substantial entry (not "none") AND this chapter appears to move in a meaningfully different direction from it, not just add consistent detail. Your voice is direct, selective, specific, and slightly pointed. Never use the words "inconsistency", "error", "problem", or "contradiction". Say "evolving", "different direction", "moving toward something new". Not alarmed. Precise. Never use em dashes. Respond ONLY with a JSON object.`,
